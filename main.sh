...
main() {
        ...
        *)
            ...
            if [[ -f "$HOME/.claudebox/default-flags" ]]; then
                ...
                if [[ ${#saved_flags[@]} -gt 0 ]]; then
                    ...
                    parse_cli_args "${original_args[@]+"${original_args[@]}"}" "${saved_flags[@]+"${saved_flags[@]}"}"
                    process_host_flags
    ...
    if [[ -z "${CLI_SCRIPT_COMMAND:-}" ]] && [[ -n "$first_arg" ]] && [[ "$first_arg" != -* ]]; then
        suggested_command=$(get_script_command_suggestion "$first_arg" 2>/dev/null || printf '')
        if [[ -n "$suggested_command" ]]; then
            error "Unknown ClaudeBox command: $first_arg
Did you mean: claudebox $suggested_command?"
        fi
    fi
        ...
        if [[ -n "${PROJECT_SLOT_DIR:-}" ]]; then
            ...
            if [[ -f "$HOME/.claudebox/default-flags" ]]; then
                ...
                if [[ ${#saved_flags[@]} -gt 0 ]]; then
                    ...
                    local all_args=("${original_args[@]+"${original_args[@]}"}" "${saved_flags[@]+"${saved_flags[@]}"}")
...
LABEL claudebox.project=\"$project_folder_name\""
    ...
    # Render placeholders line-by-line so multiline substitutions stay portable on macOS awk.
    local final_dockerfile=""
    local dockerfile_line=""
    while IFS= read -r dockerfile_line || [[ -n "$dockerfile_line" ]]; do
        if [[ "$dockerfile_line" =~ ^[[:space:]]*\{\{[[:space:]]*PROFILE_INSTALLATIONS[[:space:]]*\}\}[[:space:]]*$ ]]; then
            if [[ -n "$profile_installations" ]]; then
                final_dockerfile+="$profile_installations"$'\n'
            fi
        elif [[ "$dockerfile_line" =~ ^[[:space:]]*\{\{[[:space:]]*LABELS[[:space:]]*\}\}[[:space:]]*$ ]]; then
            final_dockerfile+="$labels"$'\n'
        else
            final_dockerfile+="$dockerfile_line"$'\n'
        fi
